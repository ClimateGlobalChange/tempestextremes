  ...
  for (j in 2:length(Varnames)){
    if (i<j){
      ...
      icpname<-sprintf("%s/%s/%s_ic_%s_%s_probsim.txt",output_dir,output_subdir,
                       output_prefix,Varnames[i],Varnames[j])
      prname<-sprintf("%s/%s/%s_ic_%s_%s_pearsonrmse.RData",output_dir,output_subdir,
                      output_prefix,Varnames[i],Varnames[j])
      prtxt<-sprintf("%s/%s/%s_ic_%s_%s_pearsonrmse.txt",output_dir,output_subdir,
                      output_prefix,Varnames[i],Varnames[j])
      table1<-c(table1,t1)
      table2<-c(table2,t2)
      blob1<-c(blob1,b1)
      ...
      prnames<-c(prnames,prname)
      prtxts<-c(prtxts,prtxt)
      txtico<-c(txtico,iconame)
      txticp<-c(txticp,icpname)
      data1<-c(data1,Varnames[i])
      data2<-c(data2,Varnames[j])
    }
  }
}
...
prname_string<-paste(prnames,collapse="\",\"")
prtxt_string<-paste(prtxts,collapse="\",\"")
df_icname<-ifelse(use_detectblob==TRUE,"df_merged","df_blob_per_timestep")
data1_string<-paste(data1,collapse="\",\"")
data2_string<-paste(data2,collapse="\",\"")
#Generating the report!!
ny_string<-paste(nyears,collapse=",")
...
cat("#PEARSON AND RMSE SPECS----------\n")
cat(sprintf("nrun_pr<-%d\n",numcombos))
cat("regrid<-FALSE\n")
TFtime<-ifelse(useCommonTime==TRUE,"TRUE","FALSE")
cat(sprintf("blob_file_1<-c(\"%s\")\n",blob1_string))
cat(sprintf("data_name_1<-c(\"%s\")\n",data1_string))
cat(sprintf("blob_file_2<-c(\"%s\")\n",blob2_string))
cat(sprintf("data_name_2<-c(\"%s\")\n",data2_string))
cat(sprintf("rfn_pr<-c(\"%s\")\n",prname_string))
cat(sprintf("txt_pr<-c(\"%s\")\n",prtxt_string))
cat(sprintf("useCommonTime<-%s\n",TFtime))
cat("\n")
...
TFtable<-ifelse(includeSummTable==TRUE,"TRUE","FALSE")
TFfreq<-ifelse(includeFrequencyPlots==TRUE,"TRUE","FALSE")
TFpearson<-ifelse(includePearson==TRUE,"TRUE","FALSE")
TFrmse<-ifelse(includeRMSE==TRUE,"TRUE","FALSE")
TFdens<-ifelse(includeDensityP==TRUE,"TRUE","FALSE")
TFprob<-ifelse(includeProbability==TRUE,"TRUE","FALSE")
TFspat<-ifelse(includeSpatialSimilarity==TRUE,"TRUE","FALSE")
cat(sprintf("includeSummTable<-%s\n",TFtable))
cat(sprintf("includeFrequencyPlots<-%s\n",TFfreq))
cat(sprintf("includePearson<-%s\n",TFpearson))
cat(sprintf("includeRMSE<-%s\n",TFrmse))
cat(sprintf("includeDensityP<-%s\n",TFdens))
cat(sprintf("includeProbability<-%s\n",TFprob))
cat(sprintf("includeSpatialSimilarity<-%s\n",TFspat))
sink()
