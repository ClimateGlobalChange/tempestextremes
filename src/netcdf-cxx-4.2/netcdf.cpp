                       ...
                       const NcDim* dim4)
{
    ...
    int n = num_vars();
    int varid;
    if (NcError::set_err( nc_def_var(the_id, name, (nc_type) type, ndims, dims, &varid)) != NC_NOERR) {
		return 0;
	}
...
NcFile_add_scalar_att(ushort)
NcFile_add_scalar_att(uint)
...
NcFile_add_vector_att(ushort)
NcFile_add_vector_att(uint)
...
NcBool NcDim::rename(NcToken newname)
{
	...
	NcBool ret = NcError::set_err( nc_rename_dim(the_file->id(), the_id, newname)) == NC_NOERR;
	if (ret) {
		delete [] the_name;
...
{	
	...
	if (the_file && NcError::set_err( nc_inq_dimname(the_file->id(), the_id, nam)) == NC_NOERR) {
...
{
	size_t dimlen = sz;
	if (NcError::set_err( nc_def_dim(the_file->id(), name, dimlen, &the_id)) == NC_NOERR) {
...
NcTypedComponent_as(ushort)
NcTypedComponent_as(uint)
...
NcValues* NcVar::values( void ) const
{
    ...
    case ncUInt:
		status = NcError::set_err(
		           nc_get_vara_uint(the_file->id(), the_id, crnr, edgs, 
		           (unsigned int *)valp->base()));
	break;
    case ncUShort:
		status = NcError::set_err(
		           nc_get_vara_ushort(the_file->id(), the_id, crnr, edgs, 
		           (unsigned short *)valp->base()));
	break;
    case ncNoType:
    default:
		return 0;
...
NcValues* NcVar::get_rec(NcDim* rdim, long slice)
{
    ...
    case ncUInt:
	status = NcError::set_err(
	           nc_get_vara_uint(the_file->id(), the_id, start, edge, 
	           (unsigned int *)valp->base()));
	break;
    case ncUShort:
	status = NcError::set_err(
...
NcVar_get_index(uint)
NcVar_get_index(ushort)
...
NcVar_put_array(ushort)
NcVar_put_array(uint)
...
NcVar_put_nd_array(ushort)
NcVar_put_nd_array(uint)
...
NcVar_get_array(ushort)
NcVar_get_array(uint)
...
NcVar_get_nd_array(ushort)
NcVar_get_nd_array(uint)
...
NcVar_add_scalar_att(ushort)
NcVar_add_scalar_att(uint)
...
NcBool NcVar::add_att(NcToken aname, int len, const ncbyte* vals)
{
	...
	if (NcError::set_err(
			 nc_put_att_schar (the_file->id(), the_id, aname, (nc_type) NcTypeEnum(ncbyte), len, vals)) != NC_NOERR) {
		return FALSE;
	}
	return TRUE;
}
...
NcBool NcVar::add_att(NcToken aname, int len, const char* vals)
{
	...
	if (NcError::set_err(
			 nc_put_att_text (the_file->id(), the_id, aname, len, vals)) != NC_NOERR) {
		return FALSE;
	}
	return TRUE;
}
...
NcVar_add_vector_att(ushort)
NcVar_add_vector_att(uint)
...
NcBool NcVar::sync(void)
{
	...
	char nam[NC_MAX_NAME];
	if (the_file && NcError::set_err( nc_inq_varname(the_file->id(), the_id, nam)) == NC_NOERR) {
...
NcToken NcVar::attname( int attnum ) const // caller must delete[]
{
	...
	char aname[NC_MAX_NAME];
	if (NcError::set_err( nc_inq_attname(the_file->id(), the_id, attnum, aname)) != NC_NOERR) {
		return 0;
	}
...
{
	nc_type typ;
	NcError::set_err( nc_inq_atttype(the_file->id(), the_variable->id(), the_name, &typ));
	return (NcType) typ;
}
...
NcBool NcAtt::is_valid( void ) const
{
	...
	return the_file->is_valid() &&
	    ...
	    NcError::set_err(
	        nc_inq_attid(the_file->id(), the_variable->id(), the_name, &num)) == NC_NOERR;
}
...
NcValues* NcAtt::values( void ) const
{
	...
	case ncUInt:
		status = NcError::set_err(
		           nc_get_att_uint(the_file->id(), the_variable->id(), the_name,
		           (unsigned int *)valp->base()));
	break;
	case ncUShort:
		status = NcError::set_err(
		           nc_get_att_ushort(the_file->id(), the_variable->id(), the_name,
		           (unsigned short *)valp->base()));
	break;
	case ncNoType:
	default:
...
NcBool NcAtt::rename(NcToken newname)
{
	...
	return NcError::set_err(
	         nc_rename_att(the_file->id(), the_variable->id(), the_name, newname)) == NC_NOERR;
}
...
NcBool NcAtt::remove( void )
{
	...
	return NcError::set_err(
	         nc_del_att(the_file->id(), the_variable->id(), the_name)) == NC_NOERR;
}
