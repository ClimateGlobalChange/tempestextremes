...
f<-open.nc("/Volumes/ExFAT_drive/ERA_files/ERA_1980/ERA_1980_01_vars_integ_devs.nc")
lats<-read.nc(f)$lat
lons<-read.nc(f)$lon
close.nc(f)
...
for (s in 1:length(seasons)){
  ...
  if (seasons[s]=="DJF"){
    f1name<-sprintf("%s/ERA_%04d/ERA_%04d_%02d_vars_integ_devs.nc",files_dir,ystart-1,ystart-1,mstart)
    mstart=1
  }else{
    f1name<-sprintf("%s/ERA_%04d/ERA_%04d_%02d_vars_integ_devs.nc",files_dir,ystart,ystart,mstart)
    mstart=mstart+1
  }
  f1<-open.nc(f1name)
  devs1<-read.nc(f1)$ADIPV
  close.nc(f1)
  f2name<-sprintf("%s/ERA_%04d/ERA_%04d_%02d_vars_integ_devs.nc",files_dir,ystart,ystart,mstart)
  f2<-open.nc(f2name)
  mstart=mstart+1
  f3name<-sprintf("%s/ERA_%04d/ERA_%04d_%02d_vars_integ_devs.nc",files_dir,ystart,ystart,mstart)
  f3<-open.nc(f3name)
  mstart=mstart+1
  devs2<-read.nc(f2)$ADIPV
  close.nc(f2)
  devs3<-read.nc(f3)$ADIPV
  close.nc(f3)
  #Do hist per sector
  #dvec<-c()
  for (n in 1:length(sectors)){
    ...
    if (n==1){
      png(sprintf("%s/%s_anom_dist.png",img_dir,seasons[s]),width=800,height=600)
    ...
    if (n==6){
      abline(v=1.3e-6,lwd=2,lty=2)
      dev.off()
    }
    mesg<-sprintf("%s %s: top 10 pct is %e",seasons[s],sectors[n],perc)
    print(mesg)
  }
