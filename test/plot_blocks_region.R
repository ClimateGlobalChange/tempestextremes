...
#for (region in c("NA")){
  ...
  for (season in c("MAM","JJA","SON","DJF")){
    ...
    # for reference dates
    df_tot_stitch$date_hr<-sprintf("%s_%02d",df_tot_stitch$date,df_tot_stitch$hr)
    uniq_dates<-unique(df_tot_stitch$date_hr)
    
    hgt.cols<-colorRampPalette(c("purple","blue","cyan4","green","yellow", "orange","red","darkred"))(34)
    for (u in 1:length(uniq_dates)){
      uniq=uniq_dates[u]

        t1=which(time_hr1==uniq)
        t2=which(time_hr2==uniq)
        print(t1)
        print(t2)
        if (length(t1)>0 & length(t2)>0){
           fname<-sprintf("~/pics_test_case/%s_%s/%s_%s_%02dZ_noT.png",region,season,region,time_format1[t1],time_hrs[t1])
           print(fname)
