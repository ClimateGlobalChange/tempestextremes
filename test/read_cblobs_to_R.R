...
for (i in 1:6){
  ...
  pv_anom_c<-array(NA,c(lonsize,latsize,tsz))
  z_anom_c<-array(NA,c(lonsize,latsize,tsz))
  ...
  for (y in 1980:2005){
    ...
    for (m in c("MAM","JJA","SON","DJF")){
      fname_pvanom<-paste(fdir,subdir,sprintf("ERA_%04d_%s_%s_blobs_const.nc",y,m,sectors[i]),sep="")
      #READ PV ANOM DATA
      ...
      #Save PV data
      pv_anom_c[,,start_t:end_t]<-read.nc(fpv)$PV_BLOB[lons_sub,lats_sub,]
      close.nc(fpv)
      #READ Z ANOM DATA
      fname_zanom<-paste(fdir,subdir,sprintf("ERA_%04d_%s_%s_Zblobs_const.nc",y,m,sectors[i]),sep="")
      print(sprintf("Reading %s",fname_zanom))
      #Open Z anom
      fz<-open.nc(fname_zanom)
      z_anom_c[,,start_t:end_t]<-read.nc(fz)$Z_BLOB[lons_sub,lats_sub,]
      close.nc(fz)
  ...
  image_name<-paste(sprintf("~/block_r_data/%s_pv_z_ghg_block_data_const.RData",sectors[i]))
  time_format<-as.Date(hrs/24, origin="1800-01-01")
  time_hrs<-hrs%%24
  lons_seq<-lons[lons_sub]
  lons_seq_c<-lons_c[lons_sub]
  lats_seq<-lats[lats_sub]
  save(list=c("pv_anom_c","z_anom_c","hrs","lats_sub","ghg","lats_seq","lons_seq","lons_seq_c",
