...
print("outfile name is %s"%nc_out)
...
if (sys.argv[3] == "avg"):
#create plot of standard deviation
  plt.figure(figsize=(24,12))
  m = Basemap(llcrnrlat=-90.,urcrnrlat=90.,
            llcrnrlon=0.,urcrnrlon=359.,
            resolution='c')
  ...
  m.drawparallels(lat_ticks,labels=[0,0,0,1])
  m.drawmeridians(lon_ticks,labels=[0,0,0,1])
  sdplot=m.contourf(x,y,sd,levels=np.arange(0.025,0.2,0.025),extend='max')
  cb=plt.colorbar(sdplot,shrink=0.75)
  cb.ax.tick_params(labelsize=16.)
  plt.title("%s standard deviation for %s"%(sys.argv[4],sys.argv[2]))
  sd_out=nc_split[0]+"_sd_plot.png"
  print("outfile name is %s"%sd_out)
  plt.savefig(sd_out,bbox_inches='tight')
