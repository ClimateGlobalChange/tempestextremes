...
read_netcdf<-function(flist,vlist,olist,timename,levname,latname,lonname,
	subset,minlat,maxlat,minlon,maxlon,minlev,maxlev,ncout,rdataout){
	...
	for (f in flist){
		...
		if (subset==TRUE){
			...
			#Find the indices for the subsetting
			bi<-which(lat==minlat)
			ti<-which(lat==maxlat)
			li<-which(lon==minlon)
			ri<-which(lon==maxlon)
