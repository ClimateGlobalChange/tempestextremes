...
subdirs<-c("MERRA","CFSR")
SEASON<-c("DJF","JJA")
SECTOR<-c("NA","SP")
#Output from --mergetable or --readfiles containing the formatted per-timestep blocking data
table_file_1<-sprintf("%s/MERRA/MERRA_%s_%s_merged_table.RData",input_dir,SEASON,SECTOR)
#Name of the data frame
df_name_1<-"df_merged"
#If all of your data is contained in a single file, leave this as a blank string
table_file_2<-sprintf("%s/CFSR/CFSR_%s_%s_merged_table.RData",input_dir,SEASON,SECTOR)
df_name_2<-"df_merged"
...
blob_file_1<-sprintf("%s/MERRA/MERRA_%s_%s_blobdata.RData",input_dir,SEASON,SECTOR)
#Name of the array variable for dataset 1
var_name_1<-"Z_BLOB"
#Name of the file for dataset 2
blob_file_2<-sprintf("%s/CFSR/CFSR_%s_%s_blobdata.RData",input_dir,SEASON,SECTOR)
#name of the array variable for dataset 2
var_name_2<-"Z_BLOB"

rfn_ps=sprintf("%s/%s_%s_ic_MERRA_CFSR.RData",output_dir,SEASON,SECTOR)
txt_overlaps=sprintf("%s/%s_%s_ic_MERRA_CFSR_table.txt",output_dir,SEASON,SECTOR)
txt_ps=sprintf("%s/%s_%s_ic_MERRA_CFSR_probsim.txt",output_dir,SEASON,SECTOR)
