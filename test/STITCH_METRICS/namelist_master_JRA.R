...
input_dir<-"~/BLOBSTATS_FILES/JRA"
#Location where the output data files are stored
output_dir<-"~/BLOBSTATS_FILES/JRA"
...
stitch_search_str<-sprintf("%s/JRA*_%s_%s_Z_stats.txt",input_dir,SEASON,SECTOR)
nostitch_search_str<-sprintf("%s/JRA*%s_%s_Z_stats_nostitch.txt",input_dir,SEASON,SECTOR)
search_str<-c(stitch_search_str,nostitch_search_str)

list_files<-c(sprintf("%s/JRA_stitch_list_%s",input_dir,SECTOR),sprintf("%s/JRA_nostitch_list_%s",input_dir,SECTOR))
list_rnames<-c(sprintf("%s/JRA_%s_%s_stitchtable.Rdata",output_dir,SEASON,SECTOR),
               sprintf("%s/JRA_%s_%s_nostitchtable.Rdata",output_dir,SEASON,SECTOR))
...
stitch_list<-""
detect_list<-""
rfn_merged<-sprintf("%s/JRA_%s_%s_merged_table.RData",output_dir,SEASON,SECTOR)
...
filelist_summ<-""
keepmerge<-TRUE
rfn_summ<-sprintf("%s/JRA_%s_%s_summ_table.RData",output_dir,SEASON,SECTOR)
...
########################
#USER DEFINED#
netcdf_search_str<-sprintf("%s/JRA*_%s_comb_Z_blobs.nc",input_dir,SEASON)
...
rfiles_blobdata<-sprintf("%s/JRA_%s_%s_blobdata.RData",output_dir,SEASON,SECTOR)
########################
