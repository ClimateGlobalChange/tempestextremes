...
for (i in 1:6){
  ...
  hrs2<-rep(0,tsz)
  start_t<-1
  #First: read in March through December for 1980
  for (m in 3:12){
    subdir<-"ERA_1980/"
    fname_pvanom<-paste(fdir,subdir,sprintf("ERA_1980_%02d_vars_integ.nc",m),sep="")
    #READ PV ANOM DATA
    ...
    hrs2[start_t:end_t]<-t
    pv_inst[,,start_t:end_t]<-read.nc(fpv)$IPV[lons_sub,lats_sub,]   
    close.nc(fpv)
    #READ Z ANOM DATA
    fname_zanom<-paste(fdir,subdir,sprintf("ERA_1980_%02d_vars_z500.nc",m),sep="")
  ...
  for (y in 1981:2005){
    ...
    for (m in 1:12){
      ...
      #Get end index for time axis
      end_t<-start_t+length(t)-1
      if (m==2 & (length(t)>(28*4))){
        end_t<-end_t-4
        hrs2[start_t:end_t]<-t[1:(length(t)-4)]
  ...
  for (m in 1:2){
    subdir<-"ERA_2006/"
    fname_pvanom<-paste(fdir,subdir,sprintf("ERA_2006_%02d_vars_integ.nc",m),sep="")
    #READ PV ANOM DATA
    print(sprintf("Reading %s",fname_pvanom))
    #Open PV
    fpv<-open.nc(fname_pvanom)
    ...
    close.nc(fpv)
    #READ Z ANOM DATA
    fname_zanom<-paste(fdir,subdir,sprintf("ERA_2006_%02d_vars_z500.nc",m),sep="")
    print(sprintf("Reading %s",fname_zanom))
    #Open Z anom
    fz<-open.nc(fname_zanom)
    z_inst[,,start_t:end_t]<-read.nc(fz)$Z[lons_sub,lats_sub,]
