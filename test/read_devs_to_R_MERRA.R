...
for (i in 1:6){
  ...
  pv_dev<-array(NA,c(lonsize,latsize,tsz))
  z_dev<-array(NA,c(lonsize,latsize,tsz))
  ...
  for (y in syear:eyear){
    ...
    for (m in 1:12){
      #List the dev files
      plist=list.files(path=subdir,full.names=TRUE,pattern=glob2rx(sprintf("*%04d_%02d*integ_devs.nc",y,m)))
      zlist=list.files(path=subdir,full.names=TRUE,pattern=glob2rx(sprintf("*%04d_%02d*z500_devs.nc",y,m)))
      nfiles=length(plist) 
      for (d in 1:nfiles){
        #READ Z ANOM DATA
        ...
        #Save PV data
        pv_dev[,,start_t:end_t]<-read.nc(fpv)$ADVPV[lons_sub,lats_sub,]
        close.nc(fpv)
        print(sprintf("Reading %s",fname_zanom))
        #Open Z anom
        fz<-open.nc(fname_zanom)
        z_dev[,,start_t:end_t]<-read.nc(fz)$ADZ[lons_sub,lats_sub,]
        close.nc(fz)
        #Increment next t
        start_t<-end_t+1
      }
  ...
  image_name<-paste(sprintf("~/block_r_data/%s_%s_pv_z_dev_data.RData",data,sectors[i]))
  lons_seq<-lons[lons_sub]
  lats_seq<-lats[lats_sub]
  save(list=c("pv_dev","z_dev","hrs","lats_sub","lats_seq","lons_seq",
