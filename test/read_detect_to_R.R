...
for (i in 1:6){
  ...
  for (y in 1980:2005){
    ...
    for (m in c("MAM","JJA","SON","DJF")){
      fname_pvanom<-paste(fdir,subdir,sprintf("ERA_%04d_%s_%s_blobs_nostitch.nc",y,m,sectors[i]),sep="")
      #READ PV ANOM DATA
      ...
      close.nc(fpv)
      #READ Z ANOM DATA
      fname_zanom<-paste(fdir,subdir,sprintf("ERA_%04d_%s_%s_Zblobs_nostitch.nc",y,m,sectors[i]),sep="")
      ...
      #READ Z ANOM DATA
      fname_ghg<-paste(fdir,subdir,sprintf("ERA_%04d_%s_%s_GHGblobs_nostitch.nc",y,m,sectors[i]),sep="")
  ...
  image_name<-paste(sprintf("~/block_r_data/%s_pv_z_ghg_nostitch_data.RData",sectors[i]))
  time_format_nostitch<-as.Date(hrs/24, origin="1800-01-01")
  time_hrs_nostitch<-hrs%%24
  hrs_nostitch<-hrs
  lons_seq<-lons[lons_sub]
  lons_seq_c<-lons_c[lons_sub]
  lats_seq<-lats[lats_sub]
  save(list=c("pv_anom","z_anom","hrs_nostitch","lats_sub","ghg","lats_seq","lons_seq","lons_seq_c",
              "lons_sub","lats","lons","time_format_nostitch","time_hrs_nostitch"),file=image_name)
}
