...
for ((y=$ys; y<=$ye; y++)); do
  for m in $(seq -f "%02g" 1 12); do
    ...
    outfile="ERA_"$y"/ERA_"$y"_"$m"_vars_GH.nc"
    #run integration code if file doesn't already exist
    if [ ! -e $outfile ]; then
      ~/tempestextremes/var4Dto3D --in $infile --out $outfile --hpa
    fi
...
yi=$((yint))
for m in $(seq -f "%02g" 1 12); do
  infile="ERA_"$yi"/ERA_"$yi"_"$m"_vars_GH.nc"
  echo $infile >> $DEV_INPUT
done

#Calculate block average
avg_outfile="ERA_avg/ERA_"$ys"_"$ye"_GHdailyavg.nc"
if [ ! -e $avg_outfile ]; then
  ~/tempestextremes/blockingAvg --inlist $AVG_INPUT --out $avg_outfile --varname GH --avgname AGH
fi

c=0
#Make list to input for StitchBlobs
for m in $(seq -f "%02g" 1 12); do
  devfile="ERA_"$yi"/ERA_"$yi"_"$m"_vars_GH_devs.nc"
  if [ ! -e $devfile ]; then
    c=$((c+1))
  fi
...
#if [ c -gt 0 ]; then
  ~/tempestextremes/blockingDevs --inlist $DEV_INPUT --avg $avg_outfile --varname GH --avgname AGH
#fi
