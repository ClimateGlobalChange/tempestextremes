...
for (i in 1:6){
  ...
  for (y in 1980:2005){
    ...
    for (m in 1:12){
      fname_pvanom<-paste(fdir,subdir,sprintf("ERA_%04d_%02d_vars_integ_devs.nc",y,m),sep="")
      #READ PV ANOM DATA
      ...
      #Save PV data
      pv_anom[,,start_t:end_t]<-read.nc(fpv)$ADIPV[lons_sub,lats_sub,]
      close.nc(fpv)
      #READ Z ANOM DATA
      fname_zanom<-paste(fdir,subdir,sprintf("ERA_%04d_%02d_vars_z500_devs.nc",y,m),sep="")
      print(sprintf("Reading %s",fname_zanom))
      #Open Z anom
      fz<-open.nc(fname_zanom)
      z_anom[,,start_t:end_t]<-read.nc(fz)$ADGH[lons_sub,lats_sub,]
      close.nc(fz)
      #Increment next t
      start_t<-end_t+1
    }
  }
  image_name<-paste(sprintf("~/block_r_data/%s_pv_z_data.RData",sectors[i]))
  time_format<-as.Date(hrs/24, origin="1800-01-01")
  time_hrs<-hrs%%24
  save(list=c("pv_anom","z_anom","hrs","lats_sub",
