...
\subsection{Explanation of blocking methods}
...
This program takes each input file containing the Z500 variable (specified by the \texttt{--varname} flag) and performs calculations for all gridpoints in the 35-75$^\circ$ latitude range (both hemispheres). The output file contains the variable \texttt{GHGrad}, which has a value of either 1 (gridpoint is instantaneously blocked) or 0 (not blocked).

\subsection{Anomaly Method}\label{anom}
\subsection{AvgVar}
\begin{verbatim}
