...
The output file contains time-averaged variables which have the same names as the original inputs and axes \texttt{[latitude, longitude]}. Unlike \texttt{BlockingAvg} (Section \ref{bavg}), this program does not check the time variable, so it is advisable to check the file list to ensure that only the desired files are included in the average.
